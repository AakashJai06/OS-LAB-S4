...
void sort(Process process[],int n){
    ...
    for(int i=0;i<n-1;i++){
        int min =i;
        for(int j=i+1;j<n;j++){
            if(process[min].priority > process[j].priority && count >= process[j].arrival){
                min = j;
            }
        }
        if(min!=i){
...
void print(Process process[],int n){
    ...
    printf("Process  ArrivalTime  BurstTime  Priority  Completion TurnaroundTime   WaitingTime\n");
    for(int i=0;i<n;i++){
    printf("P%d%10d%10d%10d%15d%15d%20d\n",process[i].pid,process[i].arrival,process[i].burst,process[i].priority,process[i].comp,process[i].tat,process[i].wt);
    }
...
int main(){
    ...
    scanf("%d",&n);
    Process process[n];
    printf("PID arrival Burst Priority:\n");  // Added Priority to the input prompt
    for(int i=0;i<n;i++){
        scanf("%d %d %d %d",&process[i].pid,&process[i].arrival,&process[i].burst,&process[i].priority);  // Added priority input
    }
